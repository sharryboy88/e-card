...
import PlayerIndicator from '../PlayerIndicator/PlayerIndicator.js';
...
class CardSet extends Component {
    ...
    makeBoard = () => {
        ...
        let cardSetArr = cardSet.map((card, index) =>(
            <div 
                key={uuidv4()} 
                className={`${classes.game__card} ${this.props.isActive ? classes.activePlayer : ''}`}
                id={index === mainCardIndex ? this.props.set : null}
            >
                <img 
                    onClickCapture={this.moveCardPlayer} 
                    src={card} 
                    alt="player card"
                    style={this.props.isActive ? { cursor: 'pointer' } : { opacity: 0.5 }}
                />
            </div>
        ));
    ...
    moveCardPlayer = e => {
        if(this.props.playing && this.props.isActive) {
